...
public abstract class MovingObject : MonoBehaviour
{
    ...
    protected IEnumerator SmothMovement(Vector2 end){
        float sqrRaminingDistance = ((Vector2)transform.position - end).sqrMagnitude;
        while (sqrRaminingDistance > float.Epsilon){
            Vector2 newPosition = Vector2.MoveTowards(rb2D.position,end,inverseMoveTime * Time.deltaTime);
            rb2D.MovePosition(newPosition);
            sqrRaminingDistance = ((Vector2)transform.position - end ).sqrMagnitude;
            yield return null;
        }
    }
